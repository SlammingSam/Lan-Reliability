function losingMySanity = Task_1Calc(k)
% This function calculates the values of kPackets based on the input k
% and a range of probabilities p, then plots the results.
...
for pIndex = 1:length(pValues)
    % Calculate kPackets using the formula k / (1 - p) and store in array
    kPackets(end + 1) = k / (1 - pValues(pIndex));
end 
